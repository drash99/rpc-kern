...
static int alloc_msg_buffers(struct rdma_ch_cb *cb)
{
	...
	for (i = 0; i < cb->msgbuf_cnt; i++) {
		mb_ctx = &cb->buf_ctxs[i];
		mb_ctx->recv_buf = kzalloc(cb->msgbuf_size, GFP_KERNEL); 
		...
		mb_ctx->send_buf = kzalloc(cb->msgbuf_size, GFP_KERNEL);
...
static int setup_buffers(struct rdma_ch_cb *cb)
{
	...
	for (i = 0; i < cb->msgbuf_cnt; i++) {
		...
		dma_unmap_addr_set(mb_ctx, recv_mapping, mb_ctx->recv_mr);
		if (!mb_ctx->recv_mr) {
		...
		mb_ctx->send_mr = ib_dma_map_single(cb->pd->device, mb_ctx->send_buf,
					     cb->msgbuf_size, DMA_BIDIRECTIONAL);
		dma_unmap_addr_set(mb_ctx, send_mapping, mb_ctx->send_mr);
		if (!mb_ctx->send_mr) {
